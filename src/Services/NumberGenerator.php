...
final class NumberGenerator
{
    public function next(string $key): string
    {
        ...
        $padding = max(1, (int) config('pharmacie.identifiers.padding', 6));
        $year = (int) now()->year;
        ...
        $number = DB::transaction(function () use ($key, $year): int {
            ...
            DB::table('pharmacie_sequences')->insertOrIgnore([
                'sequence_key' => $key,
                'year' => $year,
                'last_number' => 0,
                'created_at' => $now,
            ...
            $row = DB::table('pharmacie_sequences')
